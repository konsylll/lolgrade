...
class ApiService
{
    ...
    {
        $this->guzzle = new Client('https://euw.api.pvp.net/api/lol/');
    }

    public function getID($name, $server){
        $response = $this->guzzle->get($server.'/v1.4/summoner/by-name/'
    ...
    public function getParticipants($id, $serverID){
        return $this->guzzle->get('https://euw.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/'
        .$serverID.'/'.$id.'?api_key=6afe71bf-c760-46b5-9a52-f4ceeb8aa978')->send()->json()['participants'];
    }

    public function insertMaxGrades($people, $serverID){
        $allGrades = [];
        $count = 0;//TODO remove after MTP
        foreach($people as $summ){
            $grade = $this->guzzle->get('https://euw.api.pvp.net/championmastery/location/'
